<script>
    ...
    async function fetchRelayInfo() {
        try {
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch relay info, status:', response.status);
    ...
    async function createNIP98AuthEvent(method, url) {
        if (!userSigner) {
            throw new Error('No signer available for authentication. Please log in with a Nostr extension.');
        }
    ...
    async function loadEventsNeedingModeration() {
        try {
            isLoading = true;
            eventsNeedingModeration = await callNIP86API('listeventsneedingmoderation');
            console.log('Loaded events needing moderation:', eventsNeedingModeration);
        } catch (error) {
    ...
    async function banPubkey() {
        ...
        try {
            await callNIP86API('banpubkey', [newBannedPubkey, newBannedPubkeyReason]);
    ...
    async function allowPubkey() {
        ...
        try {
            await callNIP86API('allowpubkey', [newAllowedPubkey, newAllowedPubkeyReason]);
    ...
    async function banEvent() {
        ...
        try {
            await callNIP86API('banevent', [newBannedEvent, newBannedEventReason]);
    ...
    async function allowEvent() {
        ...
        try {
            await callNIP86API('allowevent', [newAllowedEvent, newAllowedEventReason]);
    ...
    async function updateRelayDescription() {
        ...
        try {
            await callNIP86API('changerelaydescription', [relayConfig.relay_description]);
    ...
    async function updateRelayConfiguration() {
        try {
            ...
            if (relayConfig.relay_name) {
                updates.push(callNIP86API('changerelayname', [relayConfig.relay_name]));
            }
            
            // Update relay description if provided
            if (relayConfig.relay_description) {
                updates.push(callNIP86API('changerelaydescription', [relayConfig.relay_description]));
            }
            
            // Update relay icon if provided
            if (relayConfig.relay_icon) {
                updates.push(callNIP86API('changerelayicon', [relayConfig.relay_icon]));
            }
    ...
    async function allowEventFromModeration(eventId) {
        try {
            await callNIP86API('allowevent', [eventId, 'Approved from moderation queue']);
            message = 'Event allowed successfully';
            messageType = 'success';
    ...
    async function banEventFromModeration(eventId) {
        try {
            await callNIP86API('banevent', [eventId, 'Banned from moderation queue']);
            message = 'Event banned successfully';
            messageType = 'success';
            await loadEventsNeedingModeration();
        } catch (error) {
...
<div class="managed-acl-container">
    ...
    <div class="tabs">
        <button class="tab {activeTab === 'pubkeys' ? 'active' : ''}" on:click={() => activeTab = 'pubkeys'}>
            Pubkeys
        </button>
        <button class="tab {activeTab === 'events' ? 'active' : ''}" on:click={() => activeTab = 'events'}>
            Events
        </button>
        <button class="tab {activeTab === 'ips' ? 'active' : ''}" on:click={() => activeTab = 'ips'}>
            IPs
        </button>
        <button class="tab {activeTab === 'kinds' ? 'active' : ''}" on:click={() => activeTab = 'kinds'}>
            Kinds
        </button>
        <button class="tab {activeTab === 'moderation' ? 'active' : ''}" on:click={() => {
        ...
        <button class="tab {activeTab === 'relay' ? 'active' : ''}" on:click={() => activeTab = 'relay'}>
            Relay Config
        </button>
    </div>
        ...
        {#if activeTab === 'pubkeys'}
            <div class="pubkeys-section">
                <div class="section">
                    ...
                    <div class="add-form">
                        <input type="text" placeholder="Pubkey (64 hex chars)" bind:value={newBannedPubkey} />
                        <input type="text" placeholder="Reason (optional)" bind:value={newBannedPubkeyReason} />
                        <button on:click={banPubkey} disabled={isLoading}>Ban Pubkey</button>
                    </div>
                    <div class="list">
                ...
                <div class="section">
                    ...
                    <div class="add-form">
                        <input type="text" placeholder="Pubkey (64 hex chars)" bind:value={newAllowedPubkey} />
                        <input type="text" placeholder="Reason (optional)" bind:value={newAllowedPubkeyReason} />
                        <button on:click={allowPubkey} disabled={isLoading}>Allow Pubkey</button>
                    </div>
                    <div class="list">
        ...
        {#if activeTab === 'events'}
            <div class="events-section">
                <div class="section">
                    <h3>Banned Events</h3>
                    <div class="add-form">
                        <input type="text" placeholder="Event ID (64 hex chars)" bind:value={newBannedEvent} />
                        <input type="text" placeholder="Reason (optional)" bind:value={newBannedEventReason} />
                        <button on:click={banEvent} disabled={isLoading}>Ban Event</button>
                    </div>
                    <div class="list">
                ...
                <div class="section">
                    ...
                    <div class="add-form">
                        <input type="text" placeholder="Event ID (64 hex chars)" bind:value={newAllowedEvent} />
                        <input type="text" placeholder="Reason (optional)" bind:value={newAllowedEventReason} />
                        <button on:click={allowEvent} disabled={isLoading}>Allow Event</button>
                    </div>
                    <div class="list">
        ...
        {#if activeTab === 'ips'}
            <div class="ips-section">
                <div class="section">
                    <h3>Blocked IPs</h3>
                    <div class="add-form">
                        <input type="text" placeholder="IP Address" bind:value={newBlockedIP} />
                        <input type="text" placeholder="Reason (optional)" bind:value={newBlockedIPReason} />
                        <button on:click={blockIP} disabled={isLoading}>Block IP</button>
                    </div>
                    <div class="list">
        ...
        {#if activeTab === 'kinds'}
            <div class="kinds-section">
                <div class="section">
                    ...
                    <div class="add-form">
                        <input type="number" placeholder="Kind number" bind:value={newAllowedKind} />
                        <button on:click={allowKind} disabled={isLoading}>Allow Kind</button>
                    </div>
                    <div class="list">
                        {#if allowedKinds && allowedKinds.length > 0}
                            {#each allowedKinds as kind}
                                <div class="list-item">
                                    ...
                                    <button class="remove-btn" on:click={() => disallowKind(kind)}>Remove</button>
                                </div>
                            {/each}
                        {:else}
                            <div class="no-items">
                                <p>No allowed kinds configured. All kinds are allowed by default.</p>
                            </div>
                        {/if}
                    </div>
                </div>
        ...
        {#if activeTab === 'moderation'}
            <div class="moderation-section">
                <div class="section">
                    ...
                    <button on:click={loadEventsNeedingModeration} disabled={isLoading}>Refresh</button>
                    <div class="list">
                        {#if eventsNeedingModeration && eventsNeedingModeration.length > 0}
                            {#each eventsNeedingModeration as item}
                                <div class="list-item">
                                    ...
                                    {/if}
                                    <div class="actions">
                                        <button on:click={() => allowEventFromModeration(item.id)}>Allow</button>
                                        <button on:click={() => banEventFromModeration(item.id)}>Ban</button>
                                    </div>
                                </div>
                            {/each}
                        {:else}
            ...
            <div class="relay-section">
                <div class="section">
                    ...
                    <div class="config-actions">
                        <button on:click={fetchRelayInfo} disabled={isLoading} class="refresh-btn">
                    ...
                    <div class="config-form">
                        <div class="form-group">
                            ...
                            <input type="text" bind:value={relayConfig.relay_name} placeholder="Enter relay name" />
                        </div>
                        <div class="form-group">
                            <label>Relay Description</label>
                            <textarea bind:value={relayConfig.relay_description} placeholder="Enter relay description"></textarea>
                        </div>
                        <div class="form-group">
                            <label>Relay Icon URL</label>
                            <input type="url" bind:value={relayConfig.relay_icon} placeholder="Enter icon URL" />
                        </div>
                        <div class="config-update-section">
                            <button on:click={updateRelayConfiguration} disabled={isLoading} class="update-all-btn">
                                {#if isLoading}
                                    ⏳ Updating...
                                {:else}
